...
public abstract class RenderEngine : IDisposable
{
    ...
    internal void DrawSettings()
    {
        ...
        ImGuiEx.HelpMarker("All drawings, unless overridden per element, will be drawn using this render engine.".Loc());
        if(LoadError == null)
        {
            ImGuiEx.Text(EColor.GreenBright, $"Render engine loaded successfully.".Loc());
            if(!Enabled)
            {
                ImGuiEx.TextWrapped(EColor.OrangeBright, "You have disabled this render engine. However, until you restart Splatoon or the game, it will remain loaded but inactive.".Loc());
            }
        }
        else
        {

...
public unsafe sealed class TranslationWorkspaceWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        if(ImGuiEx.BeginDefaultTable("SelLang", ["Language".Loc(), "~Select"], false))
        {
            ImGui.TableNextRow();
    ...
    public void EditField(InternationalString internationalStr, string str)
    {
        ...
        ImGuiEx.SetNextItemFullWidth();
        ImGui.InputTextWithHint("##input", "Use Global Value: ??".Loc(str), ref internationalStr.GetRefString(Page.TargetLanguage), 2000);
        ImGuiEx.Tooltip("Entered text will be used with game clients using language: ??. If empty, global value will be used. Global value is:\n??".Loc(Page.TargetLanguage, str));
        ImGui.PopID();
    }
}
